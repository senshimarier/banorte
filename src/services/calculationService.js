...
function projectPersonal(data, meses, id) {
  ...
  const projection = Array.from({length: meses}, (_, i) => saldoActual * (1 + 0.05 * i));  // 5% crecimiento mensual
  ...
  return { projection: projection[meses-1], recomendacion };
}

function projectEmpresa(data, trimestres, id) {
  ...
  const projection = Array.from({length: trimestres}, (_, i) => ebitda * (1 + 0.1 * i));  // 10% crecimiento trimestral
  const recomendacion = `Si aumentas la inversión en "ventas" en un 10%, el EBITDA podría subir a ${projection[trimestres-1] * 1.1}.`;
  return { projection: projection[trimestres-1], recomendacion };
}

function simulatePersonal(data, cambioGasto, categoria) {
  ...
  return { simulacion: `Excedente anual: ${excedenteAnual}`, recomendacion };
}

function simulateEmpresa(data, cambioCosto, categoria) {
  ...
  return { simulacion: `Margen neto aumenta en ${margenNetoMejora.toFixed(2)}%`, recomendacion };
}

function analyzePersonal(data, id) {
  const userData = data.filter(row => row.id === id);
  ...
  return { analisis: `Gasto en "Transporte": ${transporteGasto}`, recomendacion };
}

function analyzeEmpresa(data, id) {
  const companyData = data.filter(row => row.id === id);
  ...
  return { analisis: `Ratio de Liquidez: ${liquidez.toFixed(2)}`, recomendacion };
}

function comparePersonal(data, categoria, periodo1, periodo2) {
  ...
  return { comparacion: `Gasto en ${categoria}: ${periodo1}=${gastoP1}, ${periodo2}=${gastoP2}`, recomendacion };
}

function compareEmpresa(data, categoria, id) {
  ...
  return { comparacion: `Costo en "${categoria}": ${companyData.reduce((sum, row) => sum + row.monto, 0)}`, recomendacion };
}
